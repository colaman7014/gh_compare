...
本節（第 5～6 節，含 6.1 情境判斷自動化、6.2 實測範例、6.3 案例 A／案例 B／案例 A+B 處理方式）與第 1～4 節同屬「main＋環境快照 branch」採用模型，為此模型的實作細節補充，不另立獨立文件。若未來需要更複雜的回退情境（例如同時撤回多個互相依賴的需求），應於本節擴充，而非另開文件，以維持單一事實來源。
